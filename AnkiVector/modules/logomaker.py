...
async def logo_gen(event):
    ...
    if os.path.exists(flnme):
        await event.client.send_file(
            event.chat_id,
            file=flnme,
            caption=f"Logo by Creator Pavan",
            force_document=True,
        )
        os.remove(flnme)
        await xx.delete()
...
async def lego(event):
 ...
 try:
    ...
    fname2 = "AnkiVectorLogo.png"
    img.save(fname2, "png")
    await tbot.send_file(event.chat_id, fname2, caption="Made By Creator Pavan Network")
    if os.path.exists(fname2):
            os.remove(fname2)
 except Exception as e:
   await event.reply(f'Error Go to Help From  @CreatorPavanSupport, {e}')  
